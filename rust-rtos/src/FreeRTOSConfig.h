...
#define configUSE_LOBARO_TICKLESS_IDLE                                         \
  1 // use optimized lobaro tickless idle implementation (clock scaling + stop
    // mode for longer sleep periods) configUSE_TICKLESS_IDLE must also be 1!
...
// optional processing for stop (deep sleep) tickless mode (long tickless
// periods > 5 sec) note: clock is potentially slowed down if enough ticks could
// be suppressed! so perform sysclock check before invoke timing relevant
// function as uart tx!
